...
def get_all(project_id, project_url):
    ...
    if validate.api_result(jobs):
        limit = 25
        warning("[*] Found %s jobs for project %s.  Limiting scan depth to %s", len(jobs), project_url, limit)
        i = 0
        for job in jobs:
            info("[*] Retrieving job %s for %s which completed at %d", job['id'], project_url, job['finished_at'])
